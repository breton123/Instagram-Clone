        ...
        <div className="shadow-sm border-b bg-white sticky top-0 z-50">
            ...
            {session ? (
                ...
                <div className="relative navBtn">
                    ...
                    <div className="absolute -top-2 -right-1 w-4 h-4 bg-red-300 rounded-full flex items-center justify-center animate-pulse text-white text-sm">3</div>
                </div>
                <PlusCircleIcon onClick={() => setOpen(true)} className="navBtn" />
                <UserGroupIcon className="navBtn" />
                <HeartIcon className="navBtn" />
                <img onClick={signOut} src={session?.user?.image} alt="" className="h-10 w-10 rounded-full cursor-pointer"></img>
                </>
            ): (
